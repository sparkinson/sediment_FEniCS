...
# save files
u_file = File("results/u_fine_test.pvd") 
p_file = File("results/p_fine_test.pvd") 
c_file = File("results/c_fine_test.pvd")
c_d_file = File("results/c_d_fine_test.pvd")
...
M_c = assemble(a_M_c)
D_c = assemble(a_D_c)
AdvSink_c = assemble(a_AdvSink_c)
AdvSink_hat_c = assemble(a_AdvSink_hat_c)
...
while t < T:
    ...
    while nl_it < nl_its:
        ...
        while (Eu > picard_tol):
            ...
            b_m = inv_k*M_m*u_1.vector() - theta*K_m*u_1.vector() - theta*C_m*u_1.vector() - b_p_m + b_b_m
        ...
        A_c = inv_k*M_c + (1.0-theta)*Adv_c - (1.0-theta)*AdvSink_c + (1.0-theta)*D_c + \
            (1.0-theta)*S_c + (1.0-theta)*AdvSink_hat_c
        ...
        b_c = inv_k*M_c*c_1.vector() - theta*Adv_c*c_1.vector() + theta*AdvSink_c*c_1.vector() \
            - theta*D_c*c_1.vector() - theta*S_c*c_1.vector() - theta*AdvSink_hat_c*c_1.vector() #+ G_c
