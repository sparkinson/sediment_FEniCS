...
h = array(json.loads(file.readline()))

errors = []
for line in file.readlines():
    errors.append(array(json.loads(line)))

fig = figure(figsize = (5, 5))
fig.subplots_adjust(wspace=0.4, hspace=0.4, left=0.1, right=0.9, top=0.9, bottom=0.1)
ax = fig.add_subplot(111, axisbg='w')

ax.set_xlabel(r'$cell size$')
ax.set_ylabel(r'$error$')
# ax.set_xlim([0.0,0.03])
ax.autoscale(enable=True, axis='x')#, tight=True)
fig.tight_layout()

ax.plot(h,errors[0],'r-',label=r'$h$')
ax.plot(h,errors[2],'b-',label=r'$q$')
ax.plot(h,errors[1],'g-',label=r'$\varphi$')
ax.plot(h,errors[3],'y-',label=r'$\eta$')
ax.plot([0.5,0.25],[0.01, 0.0025],'k--',label=r'$2^{nd}$ order')
